...
model=pickle.load(open("ipl_log_prob.pkl","rb"))
st.title("Welcome")
st.divider()
...
if st.button('Predict Probability'):
    ...
    input_df = pd.DataFrame({'batting_team': [batting_team], 'bowling_team': [bowling_team], 'city': [city], 'runs_left': [runs_left], 'balls_left': [
                            balls_left], 'wickets_left': [wickets], 'target_runs': [target], 'cur_run_rate': [currentrunrate], 'req_run_rate': [requiredrunrate]})

    result = model.predict_proba(input_df)
    lossprob = result[0][0]
    winprob = result[0][1]

    st.header(batting_team+"- "+str(round(winprob*100))+"%")

    st.header(bowling_team+"- "+str(round(lossprob*100))+"%")
